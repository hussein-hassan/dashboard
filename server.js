...
server.get('/getUser/:id', (req, res) => {
  ...
  const result = userdb.users.find(_user => _user.id === userId);
  if (result) {
    res.json(result);
  } else {
...
server.delete('/removeUser/:id', (req, res) => {
  ...
  const filtered_users = userdb.users.filter(user => user.id !== userId);
});
// Register New User
...
server.post('/addOrder', (req, res) => {
  ...
  fs.readFile("./orders.json", (err, data) => {
    ...
    // Get the id of last user
    var last_item_id = data.orders[data.orders.length - 1].id;
